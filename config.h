...
static Key keys[] = {
	...
	{ 0,                            XK_Print,  spawn, 	   SHCMD("scrot %Y-%m-%d-%T-screenshot.png -e 'xclip -selection clipboard -t image/png -i $f; mv $f ~/pix/screenshots/' &>/dev/null") },
	{ MODKEY,                       XK_s,      spawn, 	   SHCMD("scrot --select %Y-%m-%d-%T-cut.png -e 'xclip -selection clipboard -t image/png -i $f; mv $f ~/pix/cuts/' &>/dev/null") },
